...
dcomp read_latext_v(int sx, int sy, int sz)
{
  ...
  if (!found)
  {
    ...
    aVeff = ltv_data.full_adj ? ltv_data.adjf_a + ltv_data.adjf_b / r + ltv_data.adjf_sigma * r : -0.385 / (A * r) + ltv_data.adj_sigma * (A * r);
  }
...
void load_latext_v(const char *filename)
{
  ...
  if (nodeID == 1)
    cout << "Loading potential lattice-format file " << filename << endl;
...
void load_external_cartes_v(const char *filename)
{
  ...
  for (int sx = 0; sx < 2 * Nmax + 2; sx++)
    l_cartes_v_data.n[sx] = new int *[2 * Nmax + 2];
  for (int sx = 0; sx < 2 * Nmax + 2; sx++)
    for (int sy = 0; sy < 2 * Nmax + 2; sy++)
      l_cartes_v_data.n[sx][sy] = new int[2 * Nmax + 2];

  l_cartes_v_data.s_Veff = new dcomp **[2 * Nmax + 2];
  for (int sx = 0; sx < 2 * Nmax + 2; sx++)
    l_cartes_v_data.s_Veff[sx] = new dcomp *[2 * Nmax + 2];
  for (int sx = 0; sx < 2 * Nmax + 2; sx++)
    for (int sy = 0; sy < 2 * Nmax + 2; sy++)
      l_cartes_v_data.s_Veff[sx][sy] = new dcomp[2 * Nmax + 2];
...
void destroy_external_cartes_v(void)
{
  ...
  for (int sx = 0; sx < 2 * Nmax + 2; sx++)
    for (int sy = 0; sy < 2 * Nmax + 2; sy++)
      delete l_cartes_v_data.n[sx][sy];
  for (int sx = 0; sx < 2 * Nmax + 2; sx++)
    delete l_cartes_v_data.n[sx];
  delete l_cartes_v_data.n;

  for (int sx = 0; sx < 2 * Nmax + 2; sx++)
    for (int sy = 0; sy < 2 * Nmax + 2; sy++)
      delete l_cartes_v_data.s_Veff[sx][sy];
  for (int sx = 0; sx < 2 * Nmax + 2; sx++)
    delete l_cartes_v_data.s_Veff[sx];
