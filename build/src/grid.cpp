...
void allocateMemory() {
	...
	w1 = new dcomp**[NUMX+2]; 
	for (int sx=0;sx<NUMX+2;sx++) w1[sx] = new dcomp*[NUM+2];
	for (int sx=0;sx<NUMX+2;sx++) for (int sy=0;sy<NUM+2;sy++) w1[sx][sy] = new dcomp[NUM+2];

	w2 = new dcomp**[NUMX+2]; 
	for (int sx=0;sx<NUMX+2;sx++) w2[sx] = new dcomp*[NUM+2];
	for (int sx=0;sx<NUMX+2;sx++) for (int sy=0;sy<NUM+2;sy++) w2[sx][sy] = new dcomp[NUM+2];
	...
	for (int sx=0;sx<NUMX+2;sx++) t_kin[sx] = new dcomp*[NUM+2];
	for (int sx=0;sx<NUMX+2;sx++) for (int sy=0;sy<NUM+2;sy++) t_kin[sx][sy] = new dcomp[NUM+2];

	if (POTSUBARRAY() ){
	  vsub = new dcomp**[NUMX+2]; 
	  for (int sx=0;sx<NUMX+2;sx++) vsub[sx] = new dcomp*[NUM+2];
	  for (int sx=0;sx<NUMX+2;sx++) for (int sy=0;sy<NUM+2;sy++) vsub[sx][sy] = new dcomp[NUM+2];
	}
