...
void setParameter(const char *key, const char *value) {
	...
	if (strcmp(key,"KINTERM")==0) KINTERM=atoi(value);
	if (strcmp(key,"SAVEPOT")==0) SAVEPOT=atoi(value);
	if (strcmp(key,"SAVEDECAY")==0) SAVEDECAY=atoi(value);
	if (strcmp(key,"EXTPOT")==0) EXTPOT=strdup(value);
	if (strcmp(key,"DATAFOLD")==0) DATAFOLD=strdup(value);
	if (strcmp(key,"POTCRITR")==0) POTCRITR=atoi(value);
	// double/float params
	...
	if (strcmp(key,"POTFLATR")==0) POTFLATR=atof(value);
	return;
}
...
void readParametersFromFile(char *filename, int echo) {
	...
	while(!paramFile.eof()) {
		...
		if (line.substr(0,commentmarker.length())!=commentmarker && line.length()>0) {
			char key[maxline]="",value[maxline]=""; int founddelim=0;
