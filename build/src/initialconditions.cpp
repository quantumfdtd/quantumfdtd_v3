...
void setInitialConditions(int seedMult)
{
	...
	switch (INITCONDTYPE) { 
		...
		if (nodeID==1) cout << "==> Initial wavefunction : Coulomb" << endl;
        	for (sx=0;sx<=NUMX+1;sx++) // CORRECTED BUG ON THE ORIGINAL CODE!!!
		      ...
		      if (r<A) r=A;
                      ...
                      switch(INITCONDAXIS){
                        case 0: ddx = dy; ddy = dz; ddz = dx; break;
                        case 1: ddx = dz; ddy = dx; ddz = dy; break;
                        case 2: ddx = dx; ddy = dy; ddz = dz; break;
			default: cout << "Invalid axis for Coulomb initial conditions" << endl; exit(1); break;
                      }

		      costheta = A*ddz/r;
                      sintheta = sqrt(1.-costheta*costheta);
                      if (ddx<A && ddy<A) ddy=A;
	  ...
	  case 5:
		// test grid
		if (nodeID==1) cout << "==> Initial wavefunction (node 1): Fourier components" << endl;
		...
		if (nodeID==1) cout << "==> End initial wavefunction (node 1) : Fourier components" << endl;
		break;
	  default:
